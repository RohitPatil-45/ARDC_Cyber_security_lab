...
@Controller
@RequestMapping("/floating_ip")
public class FloatingIPController {

	@Autowired
	private FloatingIPRepository repository;
	
	@Autowired
	private LocationRepository repositoryLocation;

	final String var_function_name = "floating_ip"; // small letter
	final String disp_function_name = "Floating IP"; // capital letter
	...
	public ModelAndView add() {
		...
		mav.addObject("pageTitle", "Add New Public IP");
		mav.addObject("action_name", var_function_name);
		mav.addObject("dcLocationList", repositoryLocation.getAllDClocations());
		FloatingIP objEnt = new FloatingIP();
		mav.addObject("objEnt", objEnt);
		return mav;
	}

	@PostMapping("/save")
	public String save(FloatingIP obj, RedirectAttributes redirectAttributes) {
		try {
			repository.save(obj);
