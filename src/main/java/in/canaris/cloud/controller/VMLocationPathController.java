...
@Controller
@RequestMapping("/vmLocationPath")
public class VMLocationPathController {

	@Autowired
	private VMLocationPathRepository repository;

	final String var_function_name = "vmLocationPath"; // small letter
	final String disp_function_name = "VmLocationPath"; // capital letter
	...
	public ModelAndView add() {
		...
		mav.addObject("action_name", var_function_name);
		VMLocationPath objEnt = new VMLocationPath();
		mav.addObject("objEnt", objEnt);
		return mav;
	}

	@PostMapping("/save")
	public String save(VMLocationPath obj, RedirectAttributes redirectAttributes) {
		try {
			repository.save(obj);
